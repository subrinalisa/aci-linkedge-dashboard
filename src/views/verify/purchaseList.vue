<template>
  <MainLayout>
    <h6 class="title">Pending Purchases List</h6>
          ...
          >
            ...
            <td class="text-center lg:w-16">
              <a-popconfirm
                ...
                ok-text="Yes"
                cancel-text="No"
                @confirm="purchaseSuspended(item?.id)"
              >
                <button
                  class="edit_btn"
                  type="button"
            ...
            <td>
              ...
              {{
                item?.purchase_date && moment(item?.purchase_date).format("ll")
              }}
            </td>
            <td class="text-right">
              <span class="md:hidden mr-1 font-bold">Total Price:</span>
              {{ Number(item?.total)?.toFixed(2) }}
            </td>
            <td class="text-right">
              <span class="md:hidden mr-1 font-bold">Gatis (%):</span>
              {{ Number(item?.getis_percent) }}
            </td>
            <td class="text-right">
              <span class="md:hidden mr-1 font-bold">Gatis (Taka):</span>
            ...
            <td>
              <span class="md:hidden mr-1 font-bold">MRR:</span>

              {{ item?.mrr?.name || "-" }}
            </td>
            <td>
              <span class="md:hidden mr-1 font-bold">Branch:</span>
              {{ item.branch?.organization_name }} - {{ item.branch?.branch }}
            </td>
            <td>
...
const { getPurchaseVerify, getPurchaseSuspended } = verifyStore;
...
onMounted(async () => {
  backupData.value = await getPurchaseVerify(page.value);
  allData.value = backupData.value?.data;
});
const handlePagination = async (pageNo) => {
  page.value = pageNo;
  const newData = await getPurchaseVerify(page.value);
  allData.value = newData?.data;
};
const purchaseSuspended = async (id) => {
  const result = await getPurchaseSuspended(id);
  if (result.success) {
