<template>
  <MainLayout>
    <h6 class="title">Pending Sales Return List</h6>
          ...
          >
            <td class="text-center lg:w-16">
              <button
                ...
                @click="
                  $router.push({
                    name: 'verify-return-sales-suspend',
                    params: { id: item?.id },
                  })
                "
              >
            ...
            <td class="text-center lg:w-16">
              <a-popconfirm
                ...
                ok-text="Yes"
                cancel-text="No"
                @confirm="saleReturnSuspended(item?.id)"
              >
                <button
                  class="edit_btn"
                  type="button"
...
const { getSalesReturnVerify, getSaleReturnSuspended } = verifyStore;
...
onMounted(async () => {
  backupData.value = await getSalesReturnVerify(page.value);
  allData.value = backupData.value?.data;
});
const handlePagination = async (pageNo) => {
  page.value = pageNo;
  const newData = await getSalesReturnVerify(page.value);
  allData.value = newData?.data;
};

const saleReturnSuspended = async (id) => {
  const result = await getSaleReturnSuspended(id);
  if (result.success) {
