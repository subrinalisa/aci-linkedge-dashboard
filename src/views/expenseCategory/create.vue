  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center">
        <div class="mb-3">
          <h6 class="font-medium">Create New Expense Category</h6>
        </div>
        <div class="mb-3">
          <button
            type="button"
      ...
      <form @submit.prevent="submitForm()">
        <div class="lg:grid grid-cols-3 gap-4 items-center">
          ...
          <v-select
            ...
            :style="
              formErrors.status
                ? 'border: 1px red solid; border-radius: 5px;'
                : ''
            "
          >
            <template #no-options>
...
const submitForm = async () => {
  const checkName = allExpenseCategory.value.some(
    (item) => item.name.toLowerCase() === form.value.name?.toLowerCase()
  );

  if (checkName) {
    showNotification("error", "Already Exist");
    form.value.name = "";
    return 0;
  }
  if (validateForm()) {
    ...
    try {
      const response = await apiClientMultiple.post(
        `/expense-category`,
        form.value
      );
...
const getAllExpenseCategory = async () => {
  const response = await apiClient.get(`/all-expense-category`);
  allExpenseCategory.value = response?.data?.expense_category;
};
</script>
