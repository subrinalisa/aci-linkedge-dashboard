...
const getReport = async () => {
  ...
  try {
    const res = await axios.get(
      `${apiBase}/report/top-prioritize-customer?date_from=${fromDate.value}&date_to=${toDate.value}`,
      config
    );
    if (res?.status == 200) {
      ...
      <table class="purchase-table">
        ...
        <tbody class="table-body">
          ...
          <tr v-for="(item, index) in allData" :key="index">
            ...
            <td>
              <span class="md:hidden mr-1 font-bold">Customer ID:</span>

              {{ item?.customer_id || "-" }}
            </td>
            <td>
              <span class="md:hidden mr-1 font-bold">Customer Name:</span>

              {{ item?.customer_name || "-" }}
            </td>
            <td class="text-right">
              <span class="md:hidden mr-1 font-bold">Total Amount:</span>
