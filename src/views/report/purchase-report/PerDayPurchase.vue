...
const getReport = async () => {
  ...
  try {
    const res = await axios.get(
      `${apiBase}/report/per-day-purchase?date_from=${fromDate.value}&date_to=${toDate.value}`,
      config
    );
    if (res?.status == 200) {
      ...
      <table class="purchase-table">
        <thead class="table-header">
          <tr>
            <th>purchase date</th>
            <th>branch</th>
            <th class="text-right">purchase discount percent</th>
          </tr>
        </thead>
        <tbody class="table-body">
          ...
          <tr v-for="(item, index) in allData" :key="index">
            <td>
              <span class="md:hidden mr-1 font-bold">Purchase Date:</span>
              {{
                `${
                  item?.purchase_date
            ...
            <td class="text-right">
              <span class="md:hidden mr-1 font-bold"
                >Purchase Discount Percent:</span
              >
              {{ Number(item?.purchase_discount_percent).toFixed(2) }}
            </td>
          </tr>
        </tbody>
      </table>
